...
class ParallelMergeSorter {
    ...
    public int[] sort() {
        ...
        pool.invoke(new ParallelSorter(0, array.length - 1));
        return array;
    }

    private class ParallelSorter extends RecursiveAction {

        private int left;
        private int right;

        public ParallelSorter(int left, int right) {
            this.left = left;
            this.right = right;
        }
        ...
        protected void compute() {
            if (left < right) {
                int mid = (left + right) / 2; // find the middle point
                ParallelSorter parallelSorterLeft = new ParallelSorter(left, mid);// sort the left half
                ParallelSorter parallelSorterRight = new ParallelSorter(mid + 1, right);// sort the right half
                invokeAll(parallelSorterLeft, parallelSorterRight);
                merge(left, mid, right); // merge the two sorted halves
            }
        }
    }
