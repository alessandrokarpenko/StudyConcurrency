package org.lessons.challanges; /**
...
class ParallelMatrixMultiplier {
    ...
    public int[][] computeProduct() throws ExecutionException, InterruptedException {
        ...
        List<RowCalculator> tasks = new ArrayList<>();
        List<Future<ReturnObject>> results;

        for (int i = 0; i < numRowsA; i++) {
            for (int j = 0; j < numColsB; j++) {
                System.out.println("Task for " + i + " and " + j);
                tasks.add(new RowCalculator(i, j));
            }
        }

        results = executorService.invokeAll(tasks);
        System.out.println("Result were got");
        executorService.shutdownNow();

        results.forEach(x-> {
            try {
                C[x.get().iRowFromA][x.get().jColumnFromB] = x.get().value;
    ...
    class ReturnObject {
        ...
        public ReturnObject(int iRowFromA, int jColumnFromB, int value) {
            this.iRowFromA = iRowFromA;
            this.jColumnFromB = jColumnFromB;
            this.value = value;
        }
    }

    class RowCalculator implements Callable<ReturnObject> {

        int iRowFromA;
        int jColumnFromB;

        public RowCalculator(int iRowFromA, int jColumnFromB) {
            this.iRowFromA = iRowFromA;
            this.jColumnFromB = jColumnFromB;
        }
        ...
        public ReturnObject call() throws Exception {
            ...
            int[] Ai = new int[numColsA];
            int[] Bi = new int[numRowsB];

            Ai = A[iRowFromA];

            for (int k = 0; k < numRowsB; k++) {
                Bi[k] = B[k][jColumnFromB];
            }
            ...
            return new ReturnObject(iRowFromA, jColumnFromB, total);
        }
    }
