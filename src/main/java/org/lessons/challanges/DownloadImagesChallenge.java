...
class ParallelImageDownloader {
    ...
    /* returns total bytes from downloading all images in imageNumbers array */
    public int downloadAll() throws InterruptedException {
        // YOUR CODE GOES HERE //

        List<SingleDownloader> tasks = Arrays.stream(imageNumbers).mapToObj(SingleDownloader::new).collect(Collectors.toList());
        ...
        return futures.stream().mapToInt(x-> {
            try {
                return x.get();
    ...
    class SingleDownloader implements Callable<Integer> {

        int imageNumber;

        public SingleDownloader(int imageNumber) {
            this.imageNumber = imageNumber;
        }

        @Override
        public Integer call() throws Exception {
            System.out.println("downloading for " + imageNumber);
